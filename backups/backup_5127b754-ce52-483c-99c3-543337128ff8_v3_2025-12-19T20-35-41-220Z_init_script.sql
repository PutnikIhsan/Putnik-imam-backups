...
-- Generated: 2025-12-19T20:35:42.058Z
...
CREATE TABLE IF NOT EXISTS public.user_settings (
  id UUID DEFAULT gen_random_uuid() NOT NULL,
  user_id UUID NOT NULL,
  language TEXT DEFAULT 'русский'::text NOT NULL,
  name TEXT,
  gender TEXT DEFAULT 'брат'::text,
  age_group TEXT DEFAULT 'взрослый'::text,
  islam_level TEXT DEFAULT 'начальный'::text,
  madhab TEXT DEFAULT 'ханафитский'::text,
  location_mode TEXT DEFAULT 'автоматически'::text,
  location_country TEXT,
  location_city TEXT,
  notifications TEXT DEFAULT 'включить'::text,
  screen_mode TEXT DEFAULT 'дневной'::text,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL,
  has_accepted_islam BOOLEAN,
  performs_namaz BOOLEAN,
  ...
  phone TEXT,
  enable_conversation_summary BOOLEAN DEFAULT false,
  telegram_current_section TEXT DEFAULT 'chat'::text,
  PRIMARY KEY (id)
);
...
-- =============================================
-- STEP 4: Database Functions (7 functions)
...
-- =============================================
-- STEP 5: RLS Policies (217 policies)
-- =============================================
...
CREATE POLICY "user_settings_admin_select" ON public.user_settings
  FOR SELECT
